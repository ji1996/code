...
module TXData =
  ...
  let private encodeData signature data =
    abiEncoder.EncodeWithOffsets(AbiEncodingStyle.None, signature, data)
  ...
  let makeData funcSpec args =
    if funcSpec.Kind = Fallback then (Array.empty, Array.empty) // fallback
         ...
         let struct (data, offsets) = encodeData signature (Array.map ArgData.pack args)
         let sig2 = signature.Address
         if funcSpec.Kind = Constructor then (data, offsets)
         else 
          let len = funcSpec.Signature.Length.ToString()
          ((Array.append funcSpec.Signature data), (Array.append [|len|] offsets))
