...
module Transaction =
  ...
  let concretize tx =
    ...
    // let mutable bytes = args.[0].Bytes
    // for i in 0..bytes.Length-1 do
    //   bytes.[i] <- 0xFFuy
    // let origValue = bytesToUnsignedBigInt LE bytes
    let origValue = bytesToUnsignedBigInt LE args.[0].Bytes
                    // else 0I
    let origData, origOffsets = TXData.makeData funcSpec args.[1..]
    // let value = if tx.UseAgent then UInt256 0I else UInt256 origValue
    let value = UInt256 origValue
    let data, offsets = if tx.UseAgent then TXData.makeForAgent Address.TARG_CONTRACT origValue origData
                        else (origData, origOffsets)
    // if tx.UseAgent then TXData.makeForAgent Address.TARG_CONTRACT origValue origData
                        // else (origData, origOffsets)
    // printfn "signature length = %d" funcSpec.Signature.Length
    // let finalOffsets = if tx.UseAgent then offsets |> Array.map (fun x -> x + bigint funcSpec.Signature.Length)
    //                    else offsets
    // let offsets = TXData.returnOffsets
    // printfn "TxData.offsets = %A" TXData.returnOffsets
    { From = from
      To = ``to``
      Value = value
      Data = data
