...
type Seed = {
  ...
  TXCursor : int
  // Used for seed mutation
  EdgeDist : int * bigint
  MutPos : string array
  StateInfo : string list
  // State : int array
  MutCursor : int
  // StateCursor : int
}
...
module Seed =
  ...
  let init cnstrFunc funcs =
    ...
    { Transactions = txs; TXCursor = idx; EdgeDist = (0, bigint.Zero); MutPos = Array.empty; StateInfo = List.empty; MutCursor = 0; }
  ...
  // Note: 修改private->public
  let setCursor seed newPos =
    { seed with TXCursor = newPos }

  let setNewMutInfo seed newMutPos newStateInfo = 
    { seed with MutPos = newMutPos; StateInfo = newStateInfo; MutCursor = 0; }
