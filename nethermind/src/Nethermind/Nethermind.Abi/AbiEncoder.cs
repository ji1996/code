...
namespace Nethermind.Abi
{
 [Flags]
    ...
    public class AbiEncoder : IAbiEncoder
    {
        ...
        public (byte[], string[]) EncodeWithOffsets(AbiEncodingStyle encodingStyle, AbiSignature signature, params object[] arguments)
        {
            bool packed = (encodingStyle & AbiEncodingStyle.Packed) == AbiEncodingStyle.Packed;
            List<byte[]> dynamicParts = new List<byte[]>();
            List<byte[]> headerParts = new List<byte[]>();
            BigInteger currentOffset = arguments.Length * AbiType.UInt256.LengthInBytes;
            BigInteger offset = 0;
            string[] offsets = new string[arguments.Length];
            for (int i = 0; i < arguments.Length; i++)
            {
                AbiType type = signature.Types[i];
                if (type.IsDynamic)
                {
                    byte[] bytes = AbiType.UInt256.Encode(currentOffset, packed);
                    headerParts.Add(bytes);
                    offset += bytes.Length;
                    byte[] encoded = type.Encode(arguments[i], packed);
                    currentOffset += encoded.Length;
                    dynamicParts.Add(encoded);
                ...
                else
                {
                    byte[] encoded = type.Encode(arguments[i], packed);
                    headerParts.Add(encoded);
                    offsets[i] = offset + ":" + encoded.Length;
            ...
            return (Bytes.Concat(encodedParts), offsets);
        }
