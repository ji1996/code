...
namespace Nethermind.Evm
{
    public class VirtualMachine : IVirtualMachine
    {
        ...
        public bool TraceTaint { get; set; } = false;
        ...
        // public int funcID { get; set; } = -1;
        public int txID { get; set; } = -1;
        public Dictionary<UInt256, TaintInfo> taintStorage = new Dictionary<UInt256, TaintInfo>();
        // public Dictionary<string, HashSet<int>> taintedFunctions { get; set; } = new Dictionary<string, HashSet<int>>(); 
        public Dictionary<int, HashSet<string>> taintsOfJumpis { get; set; } = new Dictionary<int, HashSet<string>>();
        // public Dictionary<int, Dictionary<string, int>> dominatorsOfJumpisInOneFunction = new Dictionary<int, Dictionary<string, int>>();
        ...
        // public SortedSet<int> UnVisitedInstrs  {get; set; } = new SortedSet<int>();
        public Dictionary<int, (BigInteger, int)> UnVisitedEdges { get; set; } = new Dictionary<int, (BigInteger, int)>();
        // PC, Op, Oprnd1, Oprnd2
        ...
        public SortedSet<(int, UInt256)> DefPCs { get; set; } = new SortedSet<(int, UInt256)>(); 
        // public Dictionary<(int, int, UInt256), int> DefUsePCs { get; set; } = new Dictionary<(int, int, UInt256), int>();
        public SortedSet<(int, UInt256)> DefUsePCs { get; set; } = new SortedSet<(int, UInt256)>();
        ...
        // private void LogUnVisitedInstr (int pc)
        // {
        //     UnVisitedInstrs.Add(pc);
        // }
        ...
        {
            // Console.WriteLine("funcID: " + funcID + " txID: " + txID);
            // inputData
            // byte[] inputData = evmState.Env.InputData;

            bool isTrace = _logger.IsTrace;
            bool traceOpcodes = _txTracer.IsTracingInstructions;
            BigInteger lastCompValue = 0;
            ...
            Dictionary<string, HashSet<string>> taintParamMemory = new Dictionary<string, HashSet<string>> (evmState.TaintParamMemory);
            ...
            void UpdateCurrentState()
            {
                ...
                if (isTestingTarget && TraceTaint) 
                {
                    evmState.TaintParamMemory = new Dictionary<string, HashSet<string>>(taintParamMemory);
                }
            }
            ...
            void BinOpTaintStack()
            {
                ...
                if (TraceTaint)
                {
                    newTaint.JoinAll(taintStack.Pop());
                }
                else
                {
                    newTaint.Join(taintStack.Pop());
            ...
            {
                // if(txID > 0) Console.WriteLine(txID + ":" + spot);
            ...
            // {
            //     TaintInfo taintInfo = TaintInfo.Sender();
            //     // taintInfo.Params.Add("32");
            //     taintStack.Push(taintInfo);
            // }
            ...
            while (programCounter < code.Length)
            {
			    ...
			    // Note: new added
                    ...
                    case Instruction.LT:
                    {
                        ...
                        {
                            PushOne(bytesOnStack);
                            // 2022-09-05 new added
                            lastCompValue = b - a + 1;
                        }
                        else
                        {
                            PushZero(bytesOnStack);
                            // 2022-09-03 new added
                            lastCompValue = a - b + 1;
                        }
                    ...
                    case Instruction.EQ:
                    {
                        ...
                        // 2022-09-05 new added
                        if(BigInteger.Compare(ba, bb) < 0) 
                        {
                            lastCompValue = bb - ba + 1;
                        } else 
                        {
                            lastCompValue = ba - bb + 1;
                        }
                    ...
                    case Instruction.SHA3:
                    {
                        ...
                        TaintInfo taint1 = taintStack.Pop();
                        TaintInfo taint2 = taintStack.Pop();
                        ...
                        if(TraceTaint)
                        {
                            newTaint.JoinParameter(taint1);
                            newTaint.JoinParameter(taint2);
                            foreach (KeyValuePair<string, HashSet<string>> kv in taintParamMemory) 
                            {
                                string[] arr = kv.Key.Split(":");
                                UInt256.TryParse(arr[0], out UInt256 taintedMemOffset);
                                UInt256.TryParse(arr[1], out UInt256 taintedMemLength);
                                if (!((memSrc >= taintedMemOffset + taintedMemLength) || (memSrc + memLength) <= taintedMemOffset)) 
                                {
                                    newTaint.Params.UnionWith(kv.Value);
                                }
                            }
                        }
                        taintStack.Push(newTaint);
                    ...
                    case Instruction.CALLER:
                    {
                        ...
                        // PushSenderTainted();
                        TaintInfo taintInfo = TaintInfo.Sender();
                        // Console.WriteLine("CALLER:" + txID + ":sender");
                        if(TraceTaint)
                        {
                            taintInfo.Params.Add(txID + ":sender");
                        }
                        taintStack.Push(taintInfo);
                    ...
                    case Instruction.CALLVALUE:
                    {
                        ...
                        // PushNonTainted();
                        TaintInfo taintInfo = TaintInfo.Bot();
                        if(TraceTaint)
                        {
                            taintInfo.Params.Add(txID + ":value");
                        }
                        taintStack.Push(taintInfo);
                    ...
                    case Instruction.CALLDATALOAD:
                    {
                        ...
                        if(TraceTaint)
                        {
                            taintInfo.JoinParameter(taint1);
                            // BigInteger num = txID + (src / 32) * 32;
                            // BigInteger num = BigInteger.Divide(src, 32);
                            // num = BigInteger.Multiply(num, 32);
                            // Console.WriteLine("CALLDATALOAD:" + txID + ":" + num + ":32");
                            // taintInfo.Params.Add(txID + ":" + num + ":32");
                            if (src < env.InputData.Length)
                            {
                                taintInfo.Params.Add(txID + ":" + src + ":" + Math.Min(env.InputData.Length - (int)src, 32));
                            }
                        }
                        taintStack.Push(taintInfo);
                    ...
                    case Instruction.CALLDATACOPY:
                    {
                        ...
                        TaintInfo taint1 = taintStack.Pop();
                        TaintInfo taint2 = taintStack.Pop();
                        TaintInfo taint3 = taintStack.Pop();
                        // Parameter taint
                        if (TraceTaint)
                        {
                            taint1.JoinParameter(taint2);
                            taint1.JoinParameter(taint3);
                            string keyPair = dest + ":" + length;
                            if(!taintParamMemory.ContainsKey(keyPair))
                            {
                                taintParamMemory[keyPair] = taint1.Params;
                            }
                            else
                            {
                                taintParamMemory[keyPair].UnionWith(taint1.Params);
                            }
                            if (src < env.InputData.Length)
                            {
                                taintParamMemory[keyPair].Add(txID + ":" + src + ":" + Math.Min(env.InputData.Length - (int)src, (int)length));
                            }
                        }
                        ...
                        // {
                        //     int src_int = (int)src;
                        //     int num = (src_int / 32 + i) * 32;
                        //     // UInt256 num = ((UInt256)txID) + (src / n + i) * n;
                        //     // Console.WriteLine("CALLDATACOPY:" + txID + ":" + num + ":" + length);
                        //     taintParamMemory[keyPair].Add(txID + ":" + num + ":" + length);
                        // }
                    ...
                    case Instruction.TIMESTAMP:
                    {
                        ...
                        // PushBlockTainted(instruction);
                        TaintInfo taintInfo = TaintInfo.BlockState(instruction, false);
                        // taintInfo.Params.Add("64");
                        taintStack.Push(taintInfo);
                    ...
                    case Instruction.MLOAD:
                    {
                        ...
                        if (TraceTaint)
                        {
                            valTaint.JoinParameter(idxTaint);
                            // parameter taint
                            foreach (KeyValuePair<string, HashSet<string>> kv in taintParamMemory) 
                            {
                                string[] arr = kv.Key.Split(":");
                                UInt256.TryParse(arr[0], out UInt256 taintedMemOffset);
                                UInt256.TryParse(arr[1], out UInt256 taintedMemLength);
                                if (!((memPosition >= taintedMemOffset + taintedMemLength) || (memPosition + 32) <= taintedMemOffset))
                                {
                                    valTaint.Params.UnionWith(kv.Value);
                                }
                            }
                        }
                    ...
                    case Instruction.MSTORE:
                    {
                        ...
                        if (TraceTaint)
                        {
                            valTaint.JoinParameter(taint1);
                            string keyPair = memPosition + ":32"; 
                            if (!taintParamMemory.ContainsKey(keyPair))
                            {
                                taintParamMemory[keyPair] = valTaint.Params;
                            }
                            else
                            {
                                taintParamMemory[keyPair].UnionWith(valTaint.Params);
                            }
                        }
                    ...
                    case Instruction.SLOAD:
                    {
                        ...
                        {
                            valTaint.JoinParameter(idxTaint);
                            if (DefPCMap.ContainsKey(storageIndex))
                            {
                                if (DefPCs.Contains((DefPCMap[storageIndex], storageIndex)))
                                {
                                    valTaint.Params.Add("s"+storageIndex+":defuse");
                                }
                                else
                                {
                                    valTaint.Params.Add("s"+storageIndex+":def");
                                }
                            }
                            else
                            {
                                valTaint.Params.Add("s"+storageIndex+":use");
                            }
                        }
                        ...
                        if (isTestingTarget && TraceDU && !IsDeployingTarget && DefPCMap.ContainsKey(storageIndex))
                        {
                            ...
                            if (DefPCs.Contains((DefPCMap[storageIndex], storageIndex)))
                            {
                                DefUsePCs.Add((DefPCMap[storageIndex], storageIndex));
                                // DefUsePCs.Add((DefPCMap[storageIndex], programCounter-1, storageIndex), txID);
                            }
                        }
                    ...
                    case Instruction.SSTORE:
                    {
                        ...
                        // if (TraceTaint)
                        // {
                        //     string tfKey = "s" + storageIndex;
                        //     if(txID != 0) {
                        //         if(!taintedFunctions.ContainsKey(tfKey)) 
                        //         {
                        //             HashSet<int> functions = new HashSet<int>();
                        //             functions.Add(txID);
                        //             taintedFunctions.Add(tfKey, functions);
                        //         }
                        //         else
                        //         {
                        //             taintedFunctions[tfKey].Add(txID);
                        //         }
                        //     }
                        // }
                        ...
                        if (isTestingTarget && TraceDU && !IsDeployingTarget) 
                        {
                            ...
                            DefPCs.Add((programCounter - 1, storageIndex));
                        }
...
//                            return CallResult.InvalidJumpDestination;
                        ...
                        if (isTestingTarget)
                        {
                            ...
                            int edgeHash1 = ((programCounter - 1) << 16) ^ jumpDestInt;
                            if(UnVisitedEdges.ContainsKey(edgeHash1))
                            {
                                UnVisitedEdges.Remove(edgeHash1);
                            }
                        }
...
//                                return CallResult.InvalidJumpDestination; // TODO: add a test, validating inside the condition was not covered by existing tests and fails on 61363 Ropsten
                            ...
                            if (isTestingTarget)
                            {
                                ...
                                int edgeHash = ((programCounter - 1) << 16) ^ programCounter;
                                if(!VisitedEdgeSet.Contains(edgeHash) && lastCompValue != 0) {
                                    if(UnVisitedEdges.ContainsKey(edgeHash)) {
                                        if(lastCompValue < UnVisitedEdges[edgeHash].Item1) 
                                        {
                                            UnVisitedEdges[edgeHash] = (lastCompValue, txID);
                                        }
                                    } 
                                    else 
                                    {
                                ...
                                // if (condTaint.Params.Count == 0)
                                // {
                                //     Console.WriteLine("$jumpDest is null");
                                //     foreach (int var in condTaint.VarSrcs)
                                //     {
                                //         Console.WriteLine("$jumpDest" + (programCounter - 1) + " : " + var);
                                //     }
                                // }
                                if(TraceTaint)
                                {
                                    foreach (string str in condTaint.Params)
                                    {
                                        if (!taintsOfJumpis.ContainsKey(programCounter - 1))
                                        {
                                            HashSet<string> taints = new HashSet<string>();
                                            taintsOfJumpis[programCounter - 1] = taints;
                                        }
                                        taintsOfJumpis[programCounter - 1].Add(str);
                                    }
                                }
                                
                                // if (taintsOfJumpis.ContainsKey(programCounter - 1)) {
                                //     foreach (string str in taintsOfJumpis[programCounter - 1])
                                //     {
                                //         Console.WriteLine("txID = " + txID + " pc = " + (programCounter - 1) + " taints param = " + str);
                                //     }
                                // }
                            }
                        ...
                        else // Jump not taken.
                        {
                            if (isTestingTarget)
                            {
                                ...
                                int edgeHash1 = ((programCounter - 1) << 16) ^ programCounter;
                                if(UnVisitedEdges.ContainsKey(edgeHash1))
                                {
                                    UnVisitedEdges.Remove(edgeHash1);
                                }
                                int edgeHash = ((programCounter - 1) << 16) ^ jumpDestInt;
                                if(!VisitedEdgeSet.Contains(edgeHash)) {
                                    if(UnVisitedEdges.ContainsKey(edgeHash)) {
                                        if(lastCompValue < UnVisitedEdges[edgeHash].Item1) 
                                        {
                                            UnVisitedEdges[edgeHash] = (lastCompValue, txID);
                                ...
                                // if (condTaint.Params.Count == 0)
                                // {
                                //     Console.WriteLine("$jumpProgramCounter is null");
                                //     foreach (int var in condTaint.VarSrcs)
                                //     {
                                //         Console.WriteLine("$jumpProgramCounter" + (programCounter - 1) + " : " + var);
                                //     }
                                // }
                                if (TraceTaint)
                                {
                    ...
                    case Instruction.CREATE2:
                    {
                        ...
                        if (env.CallDepth >= MaxCallDepth) // TODO: fragile ordering / potential vulnerability for different clients
                        {
                            ...
                            if (TraceTaint)
                            {
                                taintInfo.JoinParameter(valueTaint);
                                taintInfo.JoinParameter(taint1);
                                taintInfo.JoinParameter(taint2);
                                if (instruction == Instruction.CREATE2)
                                {
                                    taintInfo.JoinParameter(taint3);
                                }
                                foreach (KeyValuePair<string, HashSet<string>> kv in taintParamMemory) 
                                {
                                    string[] arr = kv.Key.Split(":");
                                    UInt256.TryParse(arr[0], out UInt256 taintedMemOffset);
                                    UInt256.TryParse(arr[1], out UInt256 taintedMemLength);
                                    if (!((memoryPositionOfInitCode >= taintedMemOffset + taintedMemLength) || (memoryPositionOfInitCode + initCodeLength) <= taintedMemOffset))
                                    {
                                        taintInfo.Params.UnionWith(kv.Value);
                                    }
                                }
                            }
                        ...
                        if (value > balance)
                        {
                            ...
                            if (TraceTaint)
                            {
                                ...
                                for (UInt256 i = 0; i < initCodeLength; i++)
                                {
                                    if(taintMemory.ContainsKey(memoryPositionOfInitCode + i))
                                    {
                                        taintInfo.JoinParameter(taintMemory[memoryPositionOfInitCode + i]);
                                    }
                                }
                            }
                    ...
                    case Instruction.STATICCALL:
                    {
                        ...
                        TaintInfo taint2 = taintStack.Pop();
                        TaintInfo taint3 = taintStack.Pop();
                        TaintInfo taint4 = taintStack.Pop();
                        TaintInfo taint5 = taintStack.Pop();
                        ...
                        {
                            taintInfo.JoinParameter(taint1);
                            taintInfo.JoinParameter(callDstTaint);
                            if (instruction == Instruction.CALL || instruction == Instruction.CALLCODE)
                            {
                                taintInfo.JoinParameter(callValueTaint);
                            }
                            taintInfo.JoinParameter(taint2);
                            taintInfo.JoinParameter(taint3);
                            taintInfo.JoinParameter(taint4);
                            taintInfo.JoinParameter(taint5);
                            // parameter taint
                            foreach (KeyValuePair<string, HashSet<string>> kv in taintParamMemory) 
                            {
                                string[] arr = kv.Key.Split(":");
                                UInt256.TryParse(arr[0], out UInt256 taintedMemOffset);
                                UInt256.TryParse(arr[1], out UInt256 taintedMemLength);
                                if (!((dataOffset >= taintedMemOffset + taintedMemLength) || (dataOffset + dataLength) <= taintedMemOffset))
                                {
                                    taintInfo.Params.UnionWith(kv.Value);
                                }
                            }
                            string keyPair = outputOffset + ":" + outputLength;
                            if (!taintParamMemory.ContainsKey(keyPair))
                            {
                                taintParamMemory[keyPair] = taintInfo.Params;
                            }
                            else
                            {
                                taintParamMemory[keyPair].UnionWith(taintInfo.Params);
                            }
                        }
